...
function handlePointerDown({ event, handleId, nodeId, onConnect, isTarget, getState, setState, isValidConnection, edgeUpdaterType, onEdgeUpdateEnd, }) {
    ...
    function onPointerMove(event) {
        ...
        const { connection, handleDomNode, isValid } = isValidHandle(event, prevClosestHandle, connectionMode, nodeId, handleId, isTarget ? 'target' : 'source', isValidConnection, doc);
    ...
    function onPointerUp(event) {
        ...
        if (prevClosestHandle) {
            const { connection, isValid } = isValidHandle(event, prevClosestHandle, connectionMode, nodeId, handleId, isTarget ? 'target' : 'source', isValidConnection, doc);
            if (isValid) {
...
const Handle = Object(react["forwardRef"])(({ type = 'source', position = Position.Top, isValidConnection = alwaysValid, isConnectable = true, id, onConnect, children, className, onMouseDown, onTouchStart, ...rest }, ref) => {
    ...
    const { connectionStartHandle, connectOnClick, noPanClassName } = useStore(selector$f, shallow["shallow"]);
...
function useDrag({ nodeRef, disabled = false, noDragClassName, handleSelector, nodeId, isSelectable, selectNodesOnDrag, }) {
    ...
    Object(react["useEffect"])(() => {
        if (nodeRef?.current) {
            const selection = Object(src_select["a" /* default */])(nodeRef.current);
            ...
            if (disabled) {
                ...
                const dragHandler = Object(drag["a" /* default */])()
                    .on('start', (event) => {
    ...
    if ((options.initial && !fitViewOnInitDone && fitViewOnInit) || !options.initial) {
        if (d3Zoom && d3Selection) {
            ...
            if (nodes.length > 0 && nodesInitialized) {
                ...
                const nextTransform = src["b" /* zoomIdentity */].translate(x, y).scale(zoom);
        ...
        if (d3Selection && d3Zoom) {
            return {
                ...
                setViewport: (transform, options) => {
                    ...
                    const nextTransform = src["b" /* zoomIdentity */]
                ...
                setCenter: (x, y, options) => {
                    ...
                    const transform = src["b" /* zoomIdentity */].translate(centerX, centerY).scale(nextZoom);
                    d3Zoom.transform(getD3Transition(d3Selection, options?.duration), transform);
                },
                fitBounds: (bounds, options) => {
                    const { width, height, minZoom, maxZoom } = store.getState();
                    const [x, y, zoom] = getTransformForBounds(bounds, width, height, minZoom, maxZoom, options?.padding ?? 0.1);
                    const transform = src["b" /* zoomIdentity */].translate(x, y).scale(zoom);
                    d3Zoom.transform(getD3Transition(d3Selection, options?.duration), transform);
                },
                project: (position) => {
...
const ZoomPane = ({ onMove, onMoveStart, onMoveEnd, onPaneContextMenu, zoomOnScroll = true, zoomOnPinch = true, panOnScroll = false, panOnScrollSpeed = 0.5, panOnScrollMode = PanOnScrollMode.Free, zoomOnDoubleClick = true, elementsSelectable, panOnDrag = true, defaultViewport, translateExtent, minZoom, maxZoom, zoomActivationKeyCode, preventScrolling = true, children, noWheelClassName, noPanClassName, }) => {
    ...
    Object(react["useEffect"])(() => {
        if (zoomPane.current) {
            ...
            const d3ZoomInstance = Object(src["a" /* zoom */])().scaleExtent([minZoom, maxZoom]).translateExtent(translateExtent);
            const selection = Object(src_select["a" /* default */])(zoomPane.current).call(d3ZoomInstance);
            const updatedTransform = src["b" /* zoomIdentity */]
            ...
            if (panOnScroll && !zoomActivationKeyPressed && !userSelectionActive) {
                d3Selection.on('wheel.zoom', (event) => {
                    ...
                    if (event.ctrlKey && zoomOnPinch) {
                        const point = Object(pointer["a" /* default */])(event);
...
const EdgeRenderer = ({ defaultMarkerColor, onlyRenderVisibleElements, elevateEdgesOnSelect, rfId, edgeTypes, noPanClassName, onEdgeUpdate, onEdgeContextMenu, onEdgeMouseEnter, onEdgeMouseMove, onEdgeMouseLeave, onEdgeClick, edgeUpdaterRadius, onEdgeDoubleClick, onEdgeUpdateStart, onEdgeUpdateEnd, children, }) => {
    const { edgesFocusable, elementsSelectable, width, height, connectionMode, nodeInternals, onError } = useStore(selector$4, shallow["shallow"]);
...
const ConnectionLine = ({ nodeId, handleType, style, type = ConnectionLineType.Bezier, CustomComponent, }) => {
    ...
    if (CustomComponent) {
        return (Object(jsx_runtime["jsx"])(CustomComponent, { connectionLineType: type, connectionLineStyle: style, fromNode: fromNode, fromHandle: fromHandle, fromX: fromX, fromY: fromY, toX: toX, toY: toY, fromPosition: fromPosition, toPosition: toPosition }));
    }
    let dAttr = '';
    const pathParams = {
        sourceX: fromX,
...
function ConnectionLineWrapper({ containerStyle, style, type, component }) {
    const { nodeId, handleType, nodesConnectable, width, height } = useStore(selector$2, shallow["shallow"]);
    const isValid = !!(nodeId && handleType && width && nodesConnectable);
    if (!isValid) {
        return null;
    }
    return (Object(jsx_runtime["jsx"])("svg", { style: containerStyle, width: width, height: height, className: "react-flow__edges react-flow__connectionline react-flow__container", children: Object(jsx_runtime["jsx"])("g", { className: "react-flow__connection", children: Object(jsx_runtime["jsx"])(ConnectionLine, { nodeId: nodeId, handleType: handleType, style: style, type: type, CustomComponent: component }) }) }));
}
...
const createRFStore = () => Object(zustand["createStore"])((set, get) => ({
    ...
    panBy: (delta) => {
        ...
        const nextTransform = src["b" /* zoomIdentity */].translate(transform[0] + delta.x, transform[1] + delta.y).scale(transform[2]);
        const extent = [
            [0, 0],
            [width, height],
        ];
...
const esm_selector = (s) => {
    if (s.nodeInternals.size === 0) {
        return false;
    }
    return s.getNodes().every((n) => n[esm_internalsSymbol]?.handleBounds !== undefined);
};
function useNodesInitialized() {
    const initialized = useStore(esm_selector);
    return initialized;
}
...
function MiniMap({ style, className, nodeStrokeColor = 'transparent', nodeColor = '#e2e2e2', nodeClassName = '', nodeBorderRadius = 5, nodeStrokeWidth = 2, maskColor = 'rgb(240, 240, 240, 0.6)', maskStrokeColor = 'none', maskStrokeWidth = 1, position = 'bottom-right', onClick, onNodeClick, pannable = false, zoomable = false, ariaLabel = 'React Flow mini map', }) {
    ...
    const onSvgNodeClick = onNodeClick
        ...
        : undefined;
    return (Object(jsx_runtime["jsx"])(Panel, { position: position, style: style, className: cc(['react-flow__minimap', className]), children: Object(jsx_runtime["jsxs"])("svg", { width: elementWidth, height: elementHeight, viewBox: `${x} ${y} ${width} ${height}`, role: "img", "aria-labelledby": labelledBy, ref: svg, onClick: onSvgClick, children: [ariaLabel && Object(jsx_runtime["jsx"])("title", { id: labelledBy, children: ariaLabel }), nodes.map((node) => {
                    const { x, y } = getNodePositionWithOrigin(node, nodeOrigin).positionAbsolute;
                    return (Object(jsx_runtime["jsx"])(MiniMapNode$1, { x: x, y: y, width: node.width, height: node.height, style: node.style, className: nodeClassNameFunc(node), color: nodeColorFunc(node), borderRadius: nodeBorderRadius, strokeColor: nodeStrokeColorFunc(node), strokeWidth: nodeStrokeWidth, shapeRendering: shapeRendering, onClick: onSvgNodeClick, id: node.id }, node.id));
...
lineWidth = 1, color, style, className, }) {
    ...
    return (Object(jsx_runtime["jsxs"])("svg", { className: cc(['react-flow__background', className]), style: {
            ...
            top: 0,
            left: 0,
        }, ref: ref, children: [Object(jsx_runtime["jsx"])("pattern", { id: patternId, x: transform[0] % scaledGap[0], y: transform[1] % scaledGap[1], width: scaledGap[0], height: scaledGap[1], patternUnits: "userSpaceOnUse", patternTransform: `translate(-${patternOffset[0]},-${patternOffset[1]})`, children: isDots ? (Object(jsx_runtime["jsx"])(DotPattern, { color: patternColor, radius: scaledSize / 2 })) : (Object(jsx_runtime["jsx"])(LinePattern, { dimensions: patternDimensions, color: patternColor, lineWidth: lineWidth })) }), Object(jsx_runtime["jsx"])("rect", { x: "0", y: "0", width: "100%", height: "100%", fill: `url(#${patternId})` })] }));
}
