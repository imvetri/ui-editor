        ...
        if (box.rtype === "content") {
            ...
            if (box.rctype === "Input" && box.boxChildren.length == 0) {
                if (box.flow) {
                    storage(box)
                    // if first render
                    // if rerender source is table index change 
                    if(boxrendercount>0&&tableindexchange){
                        box.rcvalue = boxes[box.flow.to].storage[params.storageindex][box.rcname || box.rctypename]
                        tableindexchange=false;
                    }
                    if (boxrendercount === 1) {
                        box.rcvalue = boxes[box.flow.to].storage[params.storageindex][box.rcname || box.rctypename]
                    } else {
                ...
                if (box.flow) {
                    let b= boxes[box.flow.to];
                    if (b.storagemovenext && b.storagemoveprev) {
                        setTimeout( () => {
                            boxes[b.storagemovenext].element.addEventListener("click", () => {
                                params.storageindex = params.storageindex == b.storage.length - 1 ? 0 : params.storageindex + 1;
                                tableindexchange=true;
                                rresult();
                            }
                            )
                            boxes[b.storagemoveprev].element.addEventListener("click", () => {
                                params.storageindex = params.storageindex == 0 ? b.storage.length - 1 : params.storageindex - 1
                                tableindexchange=true;
                                rresult();
                            }
                            )
